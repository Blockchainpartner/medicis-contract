...
const	ERC20_CONTRACT = '0x2260fac5e5542a773aa44fbcfedf7c193bc2c599';
...
const	ABI_DONATORI = [
	...
	'function beneficiariList(address donatore) public view returns (address[])'
];
...
async function harvest(whoPK, amount) {
	...
	const   tx = await medicisContractBenef1.harvest(amount);
...
module.exports = async function () {
	...
	**************************************************************************/
	try {
		...
		await new Contract(ABI_ERC20, ERC20_CONTRACT).methods.transfer(ADDRESSES[0], 31_024_533_404).send({from: "0x6555e1cc97d3cba6eaddebbcd7ca51d75771e0b8"})
		console.info('✅ - Success while stealing the richest address\n');
	} catch (error) {
		console.warn('⚠️ - Impossible to steal the richest address\n');
	}

	// await harvest(USERS_PK[0], 5)


	/**************************************************************************
	**	Approving the ERC20 tx for donatore
	**************************************************************************/
	await approve(MEDICIS_CONTRACT, 20_000_000_000)

	// /**************************************************************************
	// **	Performing a deposit in the Medicis Vault
	// **************************************************************************/
	await deposit(ADDRESSES[0], [[ADDRESSES[1], 10_000_000_00], [ADDRESSES[2], 10_000_000_00]])
	...
	console.log(svg)
	const list = await donatoriContract.beneficiariList(ADDRESSES[0]);
	console.log(list)
