...
import "./utils/BaseStrategy.sol";
import "./utils/IUni.sol";
import "./utils/IGenericLender.sol";
import "./utils/IWantToEth.sol";
...
contract StrategyLenderYieldOptimiser is BaseStrategy {
    ...
    address public swapRouter = address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F);
    ...
    function changeRouter(address a) public onlyGovernance {
        require(a != address(0));
        swapRouter = a;
    }
        ...
        )
    {
        ...
        if (lentAssets == 0) {
            if (_debtPayment > looseAssets) { //no position to harvest or profit to report
                _debtPayment = looseAssets; //we can only return looseAssets
            }
    ...
    function ethToWant(uint256 _amount) internal view returns (uint256) {
        ...
        uint256[] memory amounts = IUni(swapRouter).getAmountsOut(_amount, path);
